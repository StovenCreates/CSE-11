...
public class AirlineReservation {
    ...
    private static void initPassengers(String fileName) throws
            FileNotFoundException {
                ...
                while (scanner.hasNextLine()) {
                    ...
                    String passengerName = line[1].trim();
                    passengers[row] = passengerName.isEmpty() ? null : passengerName;
                }
    ...
    public static boolean book(String passengerName, int travelClass) {
        ...
        int firstRow = findFirstRow(travelClass);
        int lastRow = findLastRow(travelClass);
    ...
    public static boolean book(int row, String passengerName) {
        ...
        for (int r = firstRow; r <= lastRow; r++) {
            if (passengers[r] == null) {
                passengers[r] = passengerName;
                return true;
            }
        }
        ...
        for (int i = 0; i < passengers.length; i++) {
            if (passengers[i] == null) {
                ...
                if (travelClass == FIRST_CLASS) {
                    ...
                    economyClassAvailable++;
                }
            }
    }

        return new int[]{firstClassAvailable, businessClassAvailable, economyClassAvailable};
    }
    ...
    public static boolean upgrade(String passengerName, int upgradeClass) {
        ...
        for (int i = 0; i < passengers.length; i++) {
            if (passengers[i] != null && passengers[i].equals(passengerName)) {
                passengerIndex = i;
                currentClass = findClass(i);
                break;
            }
        }
    
        if (passengerIndex == -1 || upgradeClass <= currentClass) {
            return false;
        }
    
        int firstRow = findFirstRow(upgradeClass);
        int lastRow = findLastRow(upgradeClass);
